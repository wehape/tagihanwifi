...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="fsubscriptionedit" id="fsubscriptionedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
var currentTable = <?= JsonEncode($Page->toClientVar()) ?>;
ew.deepAssign(ew.vars, { tables: { subscription: currentTable } });
var currentPageID = ew.PAGE_ID = "edit";
var currentForm;
var fsubscriptionedit;
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->NomorSubscription->getDisplayValue($Page->NomorSubscription->EditValue))) ?>"></span>
<input type="hidden" data-table="subscription" data-field="x_NomorSubscription" data-hidden="1" name="x_NomorSubscription" id="x_NomorSubscription" value="<?= HtmlEncode($Page->NomorSubscription->CurrentValue) ?>">
</span></template>
</div></div>
    </div>
<?php } ?>
...
<div id="tpd_subscriptionedit" class="ew-custom-template"></div>
<template id="tpm_subscriptionedit">
<div id="ct_SubscriptionEdit"><div class="container mt-3" style="padding-left: 0px;">
    <h2>EDIT LANGGANAN</h2>
    <div class="row mt-5">
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fsubscriptionedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fsubscriptionedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
...
loadjs.ready(ew.applyTemplateId, function() {
    ...
    ew.applyTemplate("tpd_subscriptionedit", "tpm_subscriptionedit", "subscriptionedit", "<?= $Page->Export ?>", "subscription", ew.templateData.rows[0], <?= $Page->IsModal ? "true" : "false" ?>);
