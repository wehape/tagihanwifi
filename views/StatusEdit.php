...
<?= $Page->Pager->render() ?>
<?php } ?>
<form name="fstatusedit" id="fstatusedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
<script>
...
<input type="text" readonly class="form-control-plaintext" value="<?= HtmlEncode(RemoveHtml($Page->NomorStatus->getDisplayValue($Page->NomorStatus->EditValue))) ?>"></span>
<input type="hidden" data-table="status" data-field="x_NomorStatus" data-hidden="1" name="x_NomorStatus" id="x_NomorStatus" value="<?= HtmlEncode($Page->NomorStatus->CurrentValue) ?>">
</span></template>
</div></div>
    </div>
<?php } ?>
...
<div id="tpd_statusedit" class="ew-custom-template"></div>
<template id="tpm_statusedit">
<div id="ct_StatusEdit"><div class="container mt-3" style="padding-left: 0px;">
    <h2>EDIT STATUS</h2>
    <div class="row mt-5">
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fstatusedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fstatusedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
...
loadjs.ready(ew.applyTemplateId, function() {
    ...
    ew.applyTemplate("tpd_statusedit", "tpm_statusedit", "statusedit", "<?= $Page->Export ?>", "status", ew.templateData.rows[0], <?= $Page->IsModal ? "true" : "false" ?>);
