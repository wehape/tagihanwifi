...
$Page->showMessage();
?>
<form name="fbandwidthadd" id="fbandwidthadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<input type="<?= $Page->NomorBandwidth->getInputTextType() ?>" name="x_NomorBandwidth" id="x_NomorBandwidth" data-table="bandwidth" data-field="x_NomorBandwidth" value="<?= $Page->NomorBandwidth->EditValue ?>" size="30" maxlength="12" placeholder="<?= HtmlEncode($Page->NomorBandwidth->getPlaceHolder()) ?>" data-format-pattern="<?= HtmlEncode($Page->NomorBandwidth->formatPattern()) ?>"<?= $Page->NomorBandwidth->editAttributes() ?> aria-describedby="x_NomorBandwidth_help">
<?= $Page->NomorBandwidth->getCustomMessage() ?>
<div class="invalid-feedback"><?= $Page->NomorBandwidth->getErrorMessage() ?></div>
</span></template>
</div></div>
    </div>
<?php } ?>
...
<div id="tpd_bandwidthadd" class="ew-custom-template"></div>
<template id="tpm_bandwidthadd">
<div id="ct_BandwidthAdd"><div class="container mt-3" style="padding-left: 0px;">
    <h2>INPUT PAKET</h2>
    <div class="row mt-5">
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fbandwidthadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fbandwidthadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
...
loadjs.ready(ew.applyTemplateId, function() {
    ...
    ew.applyTemplate("tpd_bandwidthadd", "tpm_bandwidthadd", "bandwidthadd", "<?= $Page->Export ?>", "bandwidth", ew.templateData.rows[0], <?= $Page->IsModal ? "true" : "false" ?>);
