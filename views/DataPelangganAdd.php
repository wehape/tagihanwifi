...
$Page->showMessage();
?>
<form name="fdata_pelangganadd" id="fdata_pelangganadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" novalidate autocomplete="off">
...
<input type="<?= $Page->NomorPelanggan->getInputTextType() ?>" name="x_NomorPelanggan" id="x_NomorPelanggan" data-table="data_pelanggan" data-field="x_NomorPelanggan" value="<?= $Page->NomorPelanggan->EditValue ?>" size="30" maxlength="12" placeholder="<?= HtmlEncode($Page->NomorPelanggan->getPlaceHolder()) ?>" data-format-pattern="<?= HtmlEncode($Page->NomorPelanggan->formatPattern()) ?>"<?= $Page->NomorPelanggan->editAttributes() ?> aria-describedby="x_NomorPelanggan_help">
<?= $Page->NomorPelanggan->getCustomMessage() ?>
<div class="invalid-feedback"><?= $Page->NomorPelanggan->getErrorMessage() ?></div>
</span></template>
</div></div>
    </div>
<?php } ?>
...
<div id="tpd_data_pelangganadd" class="ew-custom-template"></div>
<template id="tpm_data_pelangganadd">
<div id="ct_DataPelangganAdd"><div class="container mt-3" style="padding-left: 0px;">
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fdata_pelangganadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fdata_pelangganadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
...
loadjs.ready(ew.applyTemplateId, function() {
    ...
    ew.applyTemplate("tpd_data_pelangganadd", "tpm_data_pelangganadd", "data_pelangganadd", "<?= $Page->Export ?>", "data_pelanggan", ew.templateData.rows[0], <?= $Page->IsModal ? "true" : "false" ?>);
