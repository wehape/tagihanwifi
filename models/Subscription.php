...
class Subscription extends DbTable
{
    ...
    public function __construct()
    {
        ...
        // NomorSubscription
        $this->NomorSubscription = new DbField(
            $this, // Table
            'x_NomorSubscription', // Variable name
            'NomorSubscription', // Name
            '`NomorSubscription`', // Expression
            '`NomorSubscription`', // Basic search expression
            200, // Type
            12, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`NomorSubscription`', // Virtual expression
            false, // Is virtual
        ...
        $this->NomorSubscription->InputTextType = "text";
        $this->NomorSubscription->Raw = true;
        $this->NomorSubscription->IsPrimaryKey = true; // Primary key field
        $this->NomorSubscription->Nullable = false; // NOT NULL field
        $this->NomorSubscription->Required = true; // Required field
        $this->NomorSubscription->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
        $this->Fields['NomorSubscription'] = &$this->NomorSubscription;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "subscription";
    }
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('NomorSubscription', $rs)) {
                AddFilter($where, QuotedName('NomorSubscription', $this->Dbid) . '=' . QuotedValue($rs['NomorSubscription'], $this->NomorSubscription->DataType, $this->Dbid));
            }
        }
        $filter = $curfilter ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->NomorSubscription->DbValue = $row['NomorSubscription'];
    ...
    {
        return "`NomorSubscription` = '@NomorSubscription@'";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->NomorSubscription->CurrentValue : $this->NomorSubscription->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->NomorSubscription->CurrentValue = $keys[0];
            } else {
                $this->NomorSubscription->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            $val = array_key_exists('NomorSubscription', $row) ? $row['NomorSubscription'] : null;
        } else {
            $val = !EmptyValue($this->NomorSubscription->OldValue) && !$current ? $this->NomorSubscription->OldValue : $this->NomorSubscription->CurrentValue;
        }
        if ($val === null) {
            return "0=1"; // Invalid key
        } else {
            $keyFilter = str_replace("@NomorSubscription@", AdjustSql($val, $this->Dbid), $keyFilter); // Replace key value
        }
        return $keyFilter;
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("SubscriptionList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "SubscriptionView" => $Language->phrase("View"),
            "SubscriptionEdit" => $Language->phrase("Edit"),
            "SubscriptionAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "SubscriptionView",
            Config("API_ADD_ACTION") => "SubscriptionAdd",
            Config("API_EDIT_ACTION") => "SubscriptionEdit",
            Config("API_DELETE_ACTION") => "SubscriptionDelete",
            Config("API_LIST_ACTION") => "SubscriptionList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("SubscriptionView", $parm);
        } else {
            $url = $this->keyUrl("SubscriptionView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("SubscriptionEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("SubscriptionList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("SubscriptionAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("SubscriptionList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("SubscriptionDelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"NomorSubscription\":" . VarToJson($this->NomorSubscription->CurrentValue, "string");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    public function keyUrl($url, $parm = "")
    {
        if ($this->NomorSubscription->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->NomorSubscription->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("NomorSubscription") ?? Route("NomorSubscription")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->NomorSubscription->CurrentValue = $key;
            } else {
                $this->NomorSubscription->OldValue = $key;
            }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->NomorSubscription);
        ...
        while (($row = $result->fetch()) && $recCnt < $stopRec) {
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        $doc->exportField($this->NomorSubscription);
    ...
    public function rowInserting($rsold, &$rsnew) {
    	$rsnew["NomorSubscription"] = NomorSubscription();
        if ($rsnew["JenisSubscription"] != "") {
    		$awal_jenissubscription = ucwords($rsnew["JenisSubscription"]);
    		$rsnew["JenisSubscription"] = $awal_jenissubscription;
    	}
    	return TRUE;
    }
    ...
    public function rowRendered() {
    	if (CurrentPageID() == "add" && $this->CurrentAction != "F") {
    		$this->NomorSubscription->CurrentValue = NomorSubscription();
    		$this->NomorSubscription->EditValue = $this->NomorSubscription->CurrentValue;
    		$this->NomorSubscription->ReadOnly = TRUE;
    	}
