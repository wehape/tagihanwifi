 ...
 * Page class
 */
class BandwidthEdit extends Bandwidth
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "BandwidthEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "BandwidthEdit";
    ...
    public function run()
    {
        ...
        $this->setVisibility();
        $this->NomorBandwidth->Required = false;
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("NomorBandwidth") ?? Key(0) ?? Route(2)) !== null) {
                $this->NomorBandwidth->setQueryStringValue($keyValue);
                $this->NomorBandwidth->setOldValue($this->NomorBandwidth->QueryStringValue);
            } elseif (Post("NomorBandwidth") !== null) {
                $this->NomorBandwidth->setFormValue(Post("NomorBandwidth"));
                $this->NomorBandwidth->setOldValue($this->NomorBandwidth->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("NomorBandwidth") ?? Route("NomorBandwidth")) !== null) {
                    $this->NomorBandwidth->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->NomorBandwidth->CurrentValue = null;
                }
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("BandwidthList"); // Return to list page
                        return;
                        ...
                        if ($this->NomorBandwidth->CurrentValue != null) {
                            while ($this->fetch()) {
                                if (SameString($this->NomorBandwidth->CurrentValue, $this->CurrentRow['NomorBandwidth'])) {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("BandwidthList"); // No matching record, return to list
                        return;
                    }
                } // End modal checking
                break;
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "BandwidthList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "BandwidthList") {
                            ...
                            $returnUrl = "BandwidthList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'NomorBandwidth' first before field var 'x_NomorBandwidth'
        $val = $CurrentForm->hasValue("NomorBandwidth") ? $CurrentForm->getValue("NomorBandwidth") : $CurrentForm->getValue("x_NomorBandwidth");
        if (!$this->NomorBandwidth->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->NomorBandwidth->Visible = false; // Disable update for API request
            } else {
                $this->NomorBandwidth->setFormValue($val);
            }
        }
    ...
    {
        global $CurrentForm;
        $this->NomorBandwidth->CurrentValue = $this->NomorBandwidth->FormValue;
    ...
    public function renderRow()
    {
        ...
        // NomorBandwidth
        $this->NomorBandwidth->RowCssClass = "row";
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->NomorBandwidth->Visible && $this->NomorBandwidth->Required) {
                if (!$this->NomorBandwidth->IsDetailKey && EmptyValue($this->NomorBandwidth->FormValue)) {
                    $this->NomorBandwidth->addErrorMessage(str_replace("%s", $this->NomorBandwidth->caption(), $this->NomorBandwidth->RequiredErrorMessage));
                }
            }
    ...
    protected function editRow()
    {
        ...
        // Check field with unique index (NomorBandwidth)
        if ($this->NomorBandwidth->CurrentValue != "") {
            $filterChk = "(`NomorBandwidth` = '" . AdjustSql($this->NomorBandwidth->CurrentValue, $this->Dbid) . "')";
            ...
            if ($rsChk->fetch()) {
                $idxErrMsg = str_replace("%f", $this->NomorBandwidth->caption(), $Language->phrase("DupIndex"));
                $idxErrMsg = str_replace("%v", $this->NomorBandwidth->CurrentValue, $idxErrMsg);
                $this->setFailureMessage($idxErrMsg);
                return false;
            }
        }

        // Check field with unique index (Bandwidth)
        if ($this->Bandwidth->CurrentValue != "") {
            $filterChk = "(`Bandwidth` = '" . AdjustSql($this->Bandwidth->CurrentValue, $this->Dbid) . "')";
            $filterChk .= " AND NOT (" . $filter . ")";
            $this->CurrentFilter = $filterChk;
            $sqlChk = $this->getCurrentSql();
            $rsChk = $conn->executeQuery($sqlChk);
            ...
            if ($rsChk->fetch()) {
                $idxErrMsg = str_replace("%f", $this->Bandwidth->caption(), $Language->phrase("DupIndex"));
                $idxErrMsg = str_replace("%v", $this->Bandwidth->CurrentValue, $idxErrMsg);
                $this->setFailureMessage($idxErrMsg);
                return false;
            }
        }
