 ...
 * Page class
 */
class BroadcastAdd extends Broadcast
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "BroadcastAdd";
    ...
    // CSS class/style
    public $CurrentPageName = "BroadcastAdd";
        ...
        switch ($this->CurrentAction) {
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "BroadcastView") {
    ...
    {
        $this->Tahun->DefaultValue = $this->Tahun->getDefault(); // PHP
        $this->Tahun->OldValue = $this->Tahun->DefaultValue;
        $this->Bulan->DefaultValue = $this->Bulan->getDefault(); // PHP
        $this->Bulan->OldValue = $this->Bulan->DefaultValue;
        $this->Tanggal->DefaultValue = $this->Tanggal->getDefault(); // PHP
        $this->Tanggal->OldValue = $this->Tanggal->DefaultValue;
        $this->Nilai->DefaultValue = $this->Nilai->getDefault(); // PHP
        $this->Nilai->OldValue = $this->Nilai->DefaultValue;
    }
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if (!$this->NomorBC->Raw) {
                $this->NomorBC->CurrentValue = HtmlDecode($this->NomorBC->CurrentValue);
            }
            $this->NomorBC->EditValue = HtmlEncode($this->NomorBC->CurrentValue);
            $this->NomorBC->PlaceHolder = RemoveHtml($this->NomorBC->caption());
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($this->NomorBC->CurrentValue != "") { // Check field with unique index
            $filter = "(`NomorBC` = '" . AdjustSql($this->NomorBC->CurrentValue, $this->Dbid) . "')";
        ...
        if ($insertRow && $this->ValidateKey && strval($rsnew['NomorBC']) == "") {
    ...
    protected function getAddRow()
    {
        ...
        // NomorBC
        $this->NomorBC->setDbValueDef($rsnew, $this->NomorBC->CurrentValue, false);

        // Tahun
        $this->Tahun->CurrentValue = $this->Tahun->getAutoUpdateValue(); // PHP
        $this->Tahun->setDbValueDef($rsnew, $this->Tahun->CurrentValue);

        // Bulan
        $this->Bulan->CurrentValue = $this->Bulan->getAutoUpdateValue(); // PHP
        $this->Bulan->setDbValueDef($rsnew, $this->Bulan->CurrentValue);
        ...
        // Tagihan
        $this->Tagihan->setDbValueDef($rsnew, $this->Tagihan->CurrentValue, false);
    ...
    {
        if (isset($row['NomorBC'])) { // NomorBC
            $this->NomorBC->setFormValue($row['NomorBC']);
        }
        if (isset($row['Tahun'])) { // Tahun
            $this->Tahun->setFormValue($row['Tahun']);
        }
        if (isset($row['Bulan'])) { // Bulan
            $this->Bulan->setFormValue($row['Bulan']);
        }
        ...
        if (isset($row['Tagihan'])) { // Tagihan
            $this->Tagihan->setFormValue($row['Tagihan']);
        }
