...
/**
 * Table class for broadcast
 */
class Broadcast extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function deleteSql(&$rs, $where = "", $curfilter = true)
    {
        ...
        if ($rs) {
            if (array_key_exists('NomorBC', $rs)) {
                AddFilter($where, QuotedName('NomorBC', $this->Dbid) . '=' . QuotedValue($rs['NomorBC'], $this->NomorBC->DataType, $this->Dbid));
            }
        }
        $filter = $curfilter ? $this->CurrentFilter : "";
        AddFilter($filter, $where);
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->NomorBC->DbValue = $row['NomorBC'];
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("BroadcastList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "BroadcastView" => $Language->phrase("View"),
            "BroadcastEdit" => $Language->phrase("Edit"),
            "BroadcastAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "BroadcastView",
            Config("API_ADD_ACTION") => "BroadcastAdd",
            Config("API_EDIT_ACTION") => "BroadcastEdit",
            Config("API_DELETE_ACTION") => "BroadcastDelete",
            Config("API_LIST_ACTION") => "BroadcastList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("BroadcastView", $parm);
        } else {
            $url = $this->keyUrl("BroadcastView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("BroadcastEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("BroadcastList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("BroadcastAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("BroadcastList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("BroadcastDelete", $parm);
        }
    }
    ...
    public function renderEditRow()
    {
        ...
        $this->JenisSubscription->EditValue = $this->JenisSubscription->CurrentValue;
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    $doc->exportCaption($this->NomorBC);
    ...
    public function rowInserting($rsold, &$rsnew) {
    	$rsnew["NomorBC"] = NomorBC();
        $rsnew["Status"] = AutoUnPaidStat();
    	return TRUE;
    }
    ...
    public function rowRendered() {
    	if (CurrentPageID() == "add" && $this->CurrentAction != "F") {
    		$this->NomorBC->CurrentValue = NomorBC();
    		$this->NomorBC->EditValue = $this->NomorBC->CurrentValue;
    		$this->NomorBC->ReadOnly = TRUE;
            $this->Tanggal->ReadOnly = TRUE;
            $this->IP->ReadOnly = TRUE;
            $this->Bandwidth->ReadOnly = TRUE;
            $this->Tagihan->ReadOnly = TRUE;
            $this->Status->CurrentValue = AutoUnPaidStat();
