 ...
 * Page class
 */
class SubscriptionEdit extends Subscription
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "SubscriptionEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "SubscriptionEdit";
    ...
    public function run()
    {
        ...
        $this->setVisibility();
        $this->NomorSubscription->Required = false;
        ...
        if (IsApi()) {
            // Load key values
            $loaded = true;
            if (($keyValue = Get("NomorSubscription") ?? Key(0) ?? Route(2)) !== null) {
                $this->NomorSubscription->setQueryStringValue($keyValue);
                $this->NomorSubscription->setOldValue($this->NomorSubscription->QueryStringValue);
            } elseif (Post("NomorSubscription") !== null) {
                $this->NomorSubscription->setFormValue(Post("NomorSubscription"));
                $this->NomorSubscription->setOldValue($this->NomorSubscription->FormValue);
            } else {
            ...
            if (Post("action", "") !== "") {
                ...
                $loadByQuery = false;
                if (($keyValue = Get("NomorSubscription") ?? Route("NomorSubscription")) !== null) {
                    $this->NomorSubscription->setQueryStringValue($keyValue);
                    $loadByQuery = true;
                } else {
                    $this->NomorSubscription->CurrentValue = null;
                }
            ...
            if ($this->isShow()) {
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if ($this->TotalRecords <= 0) { // No record found
                        ...
                        $this->terminate("SubscriptionList"); // Return to list page
                        return;
                        ...
                        if ($this->NomorSubscription->CurrentValue != null) {
                            while ($this->fetch()) {
                                if (SameString($this->NomorSubscription->CurrentValue, $this->CurrentRow['NomorSubscription'])) {
        ...
        switch ($this->CurrentAction) {
            case "show": // Get a record to display
                if (!$this->IsModal) { // Normal edit page
                    ...
                    if (!$loaded) { // Load record based on key
                        ...
                        $this->terminate("SubscriptionList"); // No matching record, return to list
                        return;
                    }
                } // End modal checking
                break;
            case "update": // Update
                ...
                if (GetPageName($returnUrl) == "SubscriptionList") {
                ...
                if ($this->editRow()) { // Update record based on key
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "SubscriptionList") {
                            ...
                            $returnUrl = "SubscriptionList"; // Return list page content
                        }
                    }
                    if (IsJsonResponse()) {
                        $this->terminate(true);
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'NomorSubscription' first before field var 'x_NomorSubscription'
        $val = $CurrentForm->hasValue("NomorSubscription") ? $CurrentForm->getValue("NomorSubscription") : $CurrentForm->getValue("x_NomorSubscription");
        if (!$this->NomorSubscription->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->NomorSubscription->Visible = false; // Disable update for API request
            } else {
                $this->NomorSubscription->setFormValue($val);
            }
        }
    ...
    {
        global $CurrentForm;
        $this->NomorSubscription->CurrentValue = $this->NomorSubscription->FormValue;
    ...
    public function renderRow()
    {
        ...
        // NomorSubscription
        $this->NomorSubscription->RowCssClass = "row";
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            // NomorSubscription
            $this->NomorSubscription->setupEditAttributes();
            $this->NomorSubscription->EditValue = $this->NomorSubscription->CurrentValue;
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->NomorSubscription->Visible && $this->NomorSubscription->Required) {
                if (!$this->NomorSubscription->IsDetailKey && EmptyValue($this->NomorSubscription->FormValue)) {
                    $this->NomorSubscription->addErrorMessage(str_replace("%s", $this->NomorSubscription->caption(), $this->NomorSubscription->RequiredErrorMessage));
                }
            }
    ...
    protected function editRow()
    {
        ...
        // Check field with unique index (NomorSubscription)
        if ($this->NomorSubscription->CurrentValue != "") {
            $filterChk = "(`NomorSubscription` = '" . AdjustSql($this->NomorSubscription->CurrentValue, $this->Dbid) . "')";
            ...
            if ($rsChk->fetch()) {
                $idxErrMsg = str_replace("%f", $this->NomorSubscription->caption(), $Language->phrase("DupIndex"));
                $idxErrMsg = str_replace("%v", $this->NomorSubscription->CurrentValue, $idxErrMsg);
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("SubscriptionList"), "", $this->TableVar, true);
        $pageId = "edit";
